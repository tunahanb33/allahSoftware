                ...
                <form @submit.prevent="onSubmit">
                    ...
                    <div class="font-semibold">
                        <div class="flex mt-5 font-semibold">
                            <span :class="state._isValid ? 'border-[#364054]' : 'border-red-600'" class="border-r-0 border  px-[.75rem] py-[.375rem] rounded">GSM (5310000000)</span>
                            <input :class="state._isValid ? 'border-[#364054]' : 'border-red-600'" @blur="checkInput" v-model="state.gsm" class="bg-[#1d222d] border w-[80%] lg:w-[40%] px-2 focus:outline-none" type="text" name="tc">
                        </div>
                        ...
                        <p v-if="!state._isValid" class="text-red-600 mt-3">GSM Alanı Boş Bırakılamaz ve 10 Karakterden Oluşmalıdır.</p>
                    </div>
                </form>
            </div>
            <!--RESULT SECTION-->
...
const checkInput = () => {
    if(!state.value.gsm || (state.value.gsm.length != 10))
...
const onSubmit = () => {
    ...
    axios.post('/gsm', { gsm: state.value.gsm }).then(res => {
