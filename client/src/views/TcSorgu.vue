<template>
    <div class="text-white">
            <section class="w-full p-[35px] xl:mx-[190px]">
                <!--INPUT SECTION-->
                <div>
                    <h2 class="text-[24px] lg:text-[30px] font-bold text-[#e4e6e7]">TC Sorgu</h2>
                    ...
                    <table class="text-left w-full p-10">
                            <tbody>
                                    <tr v-for="result in results" :key="result.TC" class="grid grid-cols-2 text-[#f8f9f9] text-lg font-medium">
                                        ...
                                        <td class="border border-[#364054] border-t-0 py-2 px-5 col-span-1 border-r-0">İl/İlçe:</td>
                                        <td class="border border-[#364054] border-t-0 py-2 px-5 col-span-1">{{ result.NUFUSIL + ' / ' + result.NUFUSILCE }}</td>
                                    </tr>
                            </tbody>
                        </table>
                </div>
...
const onSubmit = async () => {
    ...
    const res = await request('/tc', {tc: state.value.tc}, {isLoading, _isSubmit});
    results.value = res;
};
</script>