<template>
    <div class="text-white">
        <section class="p-[10px] md:p-[35px] w-full">
            <!--INPUT SECTION-->
            <div>
                <h2 class="text-[24px] lg:text-[30px] font-bold text-[#e4e6e7]">Ad Soyad Secmen Sorgu</h2>
                ...
                <table class="text-left w-full p-10 table-auto">
                    <thead class="uppercase">
                        <tr class="w-full">
                            ...
                            <th class="border border-[#1a1f28] px-3 py-2 lg:px-5 border-r-0">Doğum Yeri</th>
                            ...
                            <th class="border border-[#1a1f28] px-3 py-2 lg:px-5">Cinsiyet</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr class="odd:bg-[#232936] text-[#f8f9f9] text-lg" v-for="result in results" :key="result.TC">
                            ...
                            <td class="border border-[#364054] px-3 py-2 lg:px-5 border-r-0">{{ (new Date(result.DOGUMTARIHI)).toLocaleDateString('tr-TR', {year: 'numeric', month: 'long', day: '2-digit'}) }}</td>
                            <td class="border border-[#364054] px-3 py-2 lg:px-5 border-r-0">{{ result.ANAADI }}</td>
                            <td class="border border-[#364054] px-3 py-2 lg:px-5 border-r-0">{{ result.BABAADI }}</td>
                            <td class="border border-[#364054] px-3 py-2 lg:px-5 border-r-0">{{ result.NUFUSILI ? result.NUFUSILI + ' / ' + result.NUFUSILCESI : 'BULUNAMADI' }}</td>
                            <td class="border border-[#364054] px-3 py-2 lg:px-5">{{ result.CINSIYETI }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
...
const onSubmit = async () => {
    const res = await request('/adsoyad-secmen', { ... state }, {isLoading, _isSubmit});
    console.log(res);
    results.value = res;
};
