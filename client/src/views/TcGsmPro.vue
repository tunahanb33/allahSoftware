<template>
    <div class="text-white">
        <section class="w-full p-[35px] xl:mx-[190px]">
            <!--INPUT SECTION-->
            <div>
                <h2 class="text-[24px] lg:text-[30px] font-bold text-[#e4e6e7]">TC Gsm Sorgu PRO</h2>
                ...
                <table class="text-left w-full p-10">
                        ...
                        <tbody>
                            <tr v-for="result in results" :key="result.GSM" class=" odd:bg-[#232936] text-[#f8f9f9] text-lg font-medium">
                                ...
                                <td class="border border-[#364054] border-t-0 py-2 px-5 border-r-0">{{ result.TEL }}</td>
                            </tr>
                        </tbody>
                    </table>
            </div>
...
const onSubmit = () => {
    ...
    axios(`/pro/?module=tc2gsm&tc=${state.value.tc}`).then(res => {
